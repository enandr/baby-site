...
app.put("/active", (req, res) => {
  connection.query("UPDATE `expanding_family`.`active_items` SET `name_suggestion` = '" + req.body.name_suggestion + "', `gender_reveal` = '" + req.body.gender_reveal + "', `progress` = '" + req.body.progress + "', `announcments` = '" + req.body.announcments + "', `events` = '" + req.body.events + "' WHERE (`id` = '1')", function (err, rows, fields) {
    if (err) throw err
    // ', `registry` = '" + req.body.registry + "', `registry_url` = '" + req.body.registry_url + "
    // var returnedRows = rows;
    // res.send(returnedRows);
    res.send('Success');
  })
